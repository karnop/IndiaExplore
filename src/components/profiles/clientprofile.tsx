        ...
        <div className={"flex flex-col h-full w-full gap-1"}>
            ...
            <div className="flex flex-col w-full gap-2 overflow-x-auto max-w-5xl mx-auto">
                ...
                <div className="flex w-full gap-1 rounded-md border-1 border-slate-200 p-1">
                    <button className={`${tab == "upcoming" ? 'bg-slate-100' : 'bg-slate-50'} rounded-md w-full md:p-2 p-1 flex items-center justify-center cursor-pointer`} onClick={() => {setTab("upcoming")}}>Upcoming Appointments</button>
                    <button className={`${tab == "past" ? 'bg-slate-100' : 'bg-slate-50'} rounded-md w-full md:p-2 p-1 flex items-center justify-center cursor-pointer`} onClick={() => {setTab("past")}}>Past Appointments</button>
                </div>
                    ...
                    <div className="border-1 border-slate-200 rounded-md flex flex-col gap-1 p-1 md:p-2">
                        {upcoming.map((order) =>
                            <div key={order.id} className="border-1 border-slate-100 hover:bg-slate-100 rounded-md p-1 md:p-2 flex flex-col gap-1">
                                ...
                                <div className="flex flex-col md:flex-row justify-between gap-2">
                                    ...
                                    <div className="flex gap-1">
                                        <div className="">Advocate : <span className="text-slate-600">{order.advocateName || "Not assigned (will be assigned soon)"}</span></div>
                                    </div>
                                </div>
                                <Divider className={"text-slate-100 h-1"}/>
                        ...
                        {/* pagination */}
                        {
                            upcoming.length > 0 ?
                                ...
                                <Button disabled={pageUp <= 1} onClick={() => setPageUp(pageUp - 1)}>
                                    Previous
                                </Button>
                                <span className="px-2 py-1 text-sm text-gray-700">
                                Page {pageUp} of {totalUp}
                            </span>
                                <Button disabled={pageUp >= totalUp} onClick={() => setPageUp(pageUp + 1)}>
                                    Next
                                </Button>
                            </div>
                                :
                                <p className="text-center text-slate-700">No Upcoming appointments</p>
                        }
                    </div>
                }
                ...
                {tab == "past" &&
                    <div className="border-1 border-slate-200 rounded-md flex flex-col gap-1 p-1 md:p-2">
                        ...
                        {/* pagination */}
                        <div className="flex justify-center gap-2 mt-2">
                            <Button disabled={pagePast <= 1} onClick={() => setPagePast(pagePast - 1)}>
                                Previous
                            </Button>
                            <span className="px-2 py-1 text-sm text-gray-700">
                                Page {pagePast} of {totalPast}
                            </span>
                            <Button disabled={pagePast >= totalPast} onClick={() => setPagePast(pagePast + 1)}>
                                Next
                            </Button>
                        </div>
                    </div>
